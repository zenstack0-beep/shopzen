...
export default function InstallmentPlans({ amount, productId, className = '', compact = false }) {
  ...
  if (!calculationAmount || !plans.length) return null;
  return <div className={`${compact ? 'mt-1 space-y-0.5' : 'mt-2 space-y-1'} ${className}`}>
    {(quotedPlans || plans).slice(0, 3).map((plan, i) => { const provider = providerMeta[plan.provider] || { label: plan.provider || 'Installment', mark: '💳' }; const months = Math.max(1, Number(plan.months) || 1); const monthly = plan.monthlyAmount != null ? Number(plan.monthlyAmount) : Math.round((Number(calculationAmount) * (1 + Number(plan.interestRate || 0) / 100) / months) * 100) / 100; return <div key={`${plan.provider}-${i}`} className="flex items-center gap-1.5 text-xs text-gray-500">{plan.providerLogo ? <img src={plan.providerLogo} alt={provider.label} className="h-4 w-auto object-contain" onError={e => { e.currentTarget.style.display='none'; }} /> : <span aria-hidden="true">{provider.mark}</span>}<span><strong>{months} × {monthly.toFixed(2)}</strong> with {provider.label}</span></div>; })}
  </div>;
}
