...
const PAYMENT_COLORS = { pending: 'bg-yellow-100 text-yellow-700', paid: 'bg-green-100 text-green-700', failed: 'bg-red-100 text-red-700' };
...
export default function AdminOrders() {
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await API.get(`/orders/admin/all?status=${statusFilter}&search=${search}&page=${page}&limit=20`);
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-4 space-y-3">
        <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search by order number or customer email..." className="form-input text-sm" />
        <div className="flex gap-2 flex-wrap">
          {ALL_STATUSES.map(s => (
            <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }} className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-all border ${statusFilter === s ? 'bg-primary text-white border-primary' : 'bg-white text-gray-600 border-gray-200 hover:border-primary hover:text-primary'}`}>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="data-table">
              ...
              <tbody>
                {orders.map(order => (
                  <tr key={order._id} className={!order.isRead ? 'bg-blue-50/30' : ''}>
                    <td>
                      <div className="flex items-center gap-2">
                        {!order.isRead && <div className="w-2 h-2 bg-primary rounded-full flex-shrink-0" />}
                        <Link to={`/admin/orders/${order._id}`} className="font-mono text-sm font-semibold text-primary hover:underline">{order.orderNumber}</Link>
                      </div>
                    </td>
                    <td>
