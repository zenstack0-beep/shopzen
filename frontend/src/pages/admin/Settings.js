...
const GATEWAY_PRESETS = {
  ...
  koko: { name: 'Koko', logo: '🟦', color: '#2563eb', fields: [{ key:'merchantId', label:'Merchant ID', type:'text' }, { key:'apiKey', label:'API Key', type:'password' }, { key:'privateKey', label:'Private Key (PEM)', type:'textarea', hint:'Paste the complete RSA private key, including BEGIN/END lines.' }, { key:'publicKey', label:'Koko Public Key (PEM)', type:'textarea', hint:'Used to verify Koko callbacks.' }, { key:'logoUrl', label:'Koko Logo', type:'image', hint:'Upload a logo to show at checkout and on installment plans.' }] },
};
...
export default function AdminSettings() {
  ...
  useEffect(() => {
    ...
    API.get('/payments/admin/all').then(r => { setGateways(r.data); const cfgs = {}; r.data.forEach(g => { cfgs[g.gateway] = { ...g.config, isEnabled: g.isEnabled, isLive: g.isLive, displayName: g.displayName }; }); setGwConfigs(cfgs); setInstallmentPlans(r.data.flatMap(g => g.config?.installmentPlans || []).filter((p, i, a) => a.findIndex(x => String(x._id || '') === String(p._id || '') && x.provider === p.provider && x.months === p.months) === i)); }).catch(() => {});
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
                            ...
                            {preset.fields.map(field => (
                              <div key={field.key}>
                                ...
                                {field.type === 'image' ? (
                                  ...
                                  <textarea rows={5} value={cfg[field.key]||''} onChange={e=>setGwConfigs(p=>({...p,[gwKey]:{...p[gwKey],[field.key]:e.target.value}}))}
                                    className="form-input text-sm font-mono" placeholder="-----BEGIN KEY-----" />
                                ) : (
            ...
            {tab === 'installments' && (
              <div><h3 className="font-semibold text-gray-900 mb-1">Installment Plans</h3><p className="text-sm text-gray-400 mb-5">Configure the Payzy plans displayed on product cards and checkout. Interest is informational and included in the customer estimate.</p>
                ...
                <div className="flex gap-2 mt-4"><button className="btn-outline text-sm" onClick={()=>setInstallmentPlans(p=>[...p,{name:`${p.length+1} months`,provider:'payzy',months:3,interestRate:0,active:true}])}>Add Plan</button><button className="btn-primary text-sm" onClick={async()=>{try{const payzy=gwConfigs.payzy||{};const koko=gwConfigs.koko||{};await Promise.all([API.put('/payments/admin/payzy',{isEnabled:payzy.isEnabled||false,isLive:payzy.isLive||false,displayName:'Payzy',config:{...payzy,installmentPlans:installmentPlans.filter(p=>(p.provider||'payzy')==='payzy')}}),API.put('/payments/admin/koko',{isEnabled:koko.isEnabled||false,isLive:koko.isLive||false,displayName:'Koko',config:{...koko,installmentPlans:installmentPlans.filter(p=>p.provider==='koko')}})]);toast.success('Installment plans saved');}catch(e){toast.error(e.response?.data?.message||'Could not save plans')}}}>Save Plans</button></div>
              </div>
            )}
