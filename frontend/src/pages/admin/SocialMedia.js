...
function PostManagementTab({ connectedPlatforms }) {
  ...
  const discardDraft=async draft=>{if(!window.confirm('Discard this unconfirmed description draft? No scheduled posts will be affected.'))return;try{await API.delete(`/social-media/schedule-drafts/${draft._id}`);if(reviewDraft?._id===draft._id){setReviewDraft(null);setDraftItems([])}toast.success('Draft discarded');await loadSchedules()}catch(error){toast.error(error.response?.data?.message||'Draft could not be discarded')}};
