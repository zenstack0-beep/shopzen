      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 space-y-3">
            {items.map(item => (
              <div key={item.cartKey||item._id}
                style={{
                  background:'var(--card-bg)',
                  opacity: removingId===(item.cartKey||item._id) ? 0 : 1,
                  transform: removingId===(item.cartKey||item._id) ? 'translateX(40px)' : 'none',
                  transition:'opacity 0.28s ease,transform 0.28s ease',
                }}
                className="rounded-2xl border p-4 flex gap-4" style2={{borderColor:'var(--card-border)'}}>
                {/* image */}
                <Link to={`/product/${item.slug}`} className="flex-shrink-0 w-22 h-22 rounded-2xl overflow-hidden bg-gray-50"
                  style={{width:88,height:88,borderRadius:16,overflow:'hidden',flexShrink:0}}>
                  <img src={item.thumbnail||item.images?.[0]||'https://placehold.co/88x88?text=+'} alt={item.name}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <Link to={`/product/${item.slug}`} className="font-bold text-gray-900 hover:opacity-70 transition-opacity text-sm sm:text-base line-clamp-2 leading-snug" style={{fontFamily:'var(--font-body)'}}>
                        {item.displayName||item.name}
                      </Link>
                      {item.category?.name && <p className="text-xs text-gray-400 mt-0.5">{item.category.name}</p>}
                    </div>
                    <button onClick={()=>handleRemove(item.cartKey||item._id)}
                  ...
                  <div className="flex items-center justify-between mt-3 flex-wrap gap-3">
                    ...
                    <div className="qty-stepper">
                      <button className="qty-stepper-btn" onClick={()=>updateQuantity(item.cartKey||item._id,item.quantity-1)} disabled={item.quantity<=1}>−</button>
                      <span className="qty-stepper-val">{item.quantity}</span>
                      <button className="qty-stepper-btn" onClick={()=>updateQuantity(item.cartKey||item._id,item.quantity+1)}>+</button>
                    </div>
                    <div className="text-right">
                      <p className="font-extrabold text-gray-900 text-lg">{sym} {((item.salePrice||item.price)*item.quantity).toLocaleString()}</p>
              ...
              <div className="p-6 space-y-3">
                {items.map(item=>(
                  <div key={item.cartKey||item._id} className="flex justify-between text-sm text-gray-500">
                    ...
                    <span className="font-semibold flex-shrink-0">{sym} {((item.salePrice||item.price)*item.quantity).toLocaleString()}</span>
                  </div>
                ))}
                <div className="border-t pt-3 space-y-2" style={{borderColor:'var(--card-border)'}}>
                  ...
                  <FreeDeliveryBar subtotal={subtotal} threshold={freeThreshold} sym={sym}/>
